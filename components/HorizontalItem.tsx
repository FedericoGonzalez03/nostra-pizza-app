...
export default function HorizontalItem({
  ...
  return (
    <View style={styles.itemContainer as ViewStyle} key={item.id}>
      <Image
        source={{ uri: item.image }}
        style={styles.itemImage as ImageStyle}
      ></Image>
      <View style={styles.itemInfo as ViewStyle}>
        <Text style={[styles.itemName as TextStyle, { fontSize: FONT_SIZES.medium * rs }]}>
          {item.name}
        </Text>
        <Text
          style={[styles.itemDescription as TextStyle, { fontSize: FONT_SIZES.small * rs }]}
        >
          {item.description}
        </Text>
      </View>
      <View style={styles.itemPriceAndController as ViewStyle}>
        <Text
          style={[
            ...
            { fontSize: (FONT_SIZES.small + 2) * rs, fontWeight: 400 },
          ]}
        >
          ${item.price}
        </Text>
        {isInCart(item) ? (
          <View
            style={[
              styles.itemController as ViewStyle,
              {
                height: 80 * rs,
            ...
            <View style={styles.itemQuantityControllerContainer as ViewStyle}>
              ...
              <TextInput
                style={[
                  ...
                  { width: 42 * rs, height: 42 * rs, fontSize: 16 * rs },
                ]}
