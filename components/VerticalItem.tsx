...
export default function VerticalItem(
	...
	return <View style={[styles.itemContainer as ViewStyle, { flexDirection: "column", justifyContent: 'space-evenly', gap: 0 }]} key={item.id}>
		<View style={{ display: "flex", alignItems: "center", flexDirection: 'row', width: '100%', gap: 15 * rs }}>
		<Image source={{uri: `data:image/jpeg;base64,${item.image}`}} style={styles.itemImage as ImageStyle}></Image>
			<View style={{ display: "flex", flexDirection: 'column', gap: 5 * rs, flexShrink: 1 }}>
		...
		<View style={{ display: "flex", flexDirection: 'row', width: '100%', justifyContent: 'space-between', alignItems: "center" }}>
			<Text style={[styles.itemPrice as TextStyle, { marginLeft: 11, fontSize: 36 * rs, fontWeight: 400 }]}>${item.price}</Text>
			{isInCart(item) ?
				<View style={styles.itemController as ViewStyle}>
					<View style={styles.itemQuantityControllerContainer as ViewStyle}>
						<View style={[styles.itemQuantityController as ViewStyle, { width: 36, height: 36 }]}>
							<Ionicons
								color={'#F5F5DC'}
								size={36}
						...
						<TextInput
							style={[styles.itemQuantityInput as TextStyle, { width: 26, height: 30, fontSize: 14 }]}
